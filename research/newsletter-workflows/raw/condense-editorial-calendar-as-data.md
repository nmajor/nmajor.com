# codex-skills `blog-calendar` — an editorial calendar / backlog as an agent-operable data structure

- URL: https://github.com/megastep/codex-skills/blob/main/blog-calendar/SKILL.md
- Date accessed: 2026-08-18
- What it is: A published skill that treats the editorial calendar as structured state an agent reads and writes: topic clusters with completion percentages, a content-decay detector driven off frontmatter `lastUpdated`, an explicit content-mix ratio, a refresh queue ordered by "next refresh date", and per-post distribution timing. This is the "idea backlog / editorial calendar as a data structure" pattern in concrete form. Cloned via `gh repo clone megastep/codex-skills`. Reproduced complete and verbatim below.

---

## VERBATIM — complete `blog-calendar/SKILL.md`
...
## NOTE ON THE STATISTIC IT CITES
The file asserts, twice and without a source link: "76% of top AI citations are from content updated within 30 days." That claim is reproduced here because it is verbatim from the skill, NOT because it is verified. No primary source was located for it during this pass.

## NOT FETCHED / GAPS
- `../blog/references/content-templates.md` (the 12 content templates) and `../blog/references/distribution-playbook.md` not reproduced.
